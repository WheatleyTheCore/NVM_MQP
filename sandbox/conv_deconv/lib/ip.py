...
def wiener_deconv(x, y, snr=20):
    ...
    noise_fft = (np.abs(x_fft)**2)/((np.abs(x_fft)**2) + (1/snr))
    ir = y_fft/x_fft 
...
def tikhonov_regularization(x, y, regularization_param):
    ...
    #..... idk, i'll have to come back to this


def ACDW(x, y, window_end=None, window_start=0):
    """
    ...
    log_fft_x = np.log(np.abs(fft_x) + 1e-10)
    log_fft_y = np.log(np.abs(fft_y) + 1e-10)

    c_x = ifft(log_fft_x).real
    c_y = ifft(log_fft_y).real

    c_ir = c_y - c_x
    c_ir_windowed = c_ir[:window_end]

    ir_FFT = fft(c_ir_windowed)
    ir = ifft(np.exp(ir_FFT)).real
